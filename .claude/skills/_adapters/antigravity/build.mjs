#!/usr/bin/env node
// adapters/antigravity/build — Claude フォーマットの SKILL.md を Antigravity Agent Skills 形式へ変換 (依存ゼロ)
//
// 使い方: node adapters/antigravity/build.mjs [--out <dir>|--out=<dir>] [--plugin <name>|--plugin=<name>]
//   --out:    出力先 (default: .agents/skills — Antigravity のプロジェクトスコープ skills ディレクトリ)
//   --plugin: 単一 plugin だけ変換 (default: 全 plugin)
//
// Antigravity Agent Skills の仕様 (Google Codelabs + agentskills.io 標準で確認):
//   - SKILL.md は Agent Skills 標準フォーマット = Claude Code と同一。frontmatter は
//     `name`(必須・小文字とハイフンのみ・親ディレクトリ名と一致) と `description`(必須) が基本。
//     optional に license / compatibility / metadata / allowed-tools(実験的) がある。
//   - **tool 名のマッピングは不要**。Antigravity 自身は SKILL.md で Claude 形式の tool を宣言せず、
//     scripts/ と汎用エージェントツール(run_command / write_to_file 等)を使う。よって本変換は
//     `allowed-tools` を出力に残さず、name + description のみ書き出す(Google の Antigravity skill 例と一致)。
//   - 配置: プロジェクト `<root>/.agents/skills/`(CLI は `.agent/skills/`)、グローバル `~/.agents/skills/`。
//   - 同梱ディレクトリ: scripts/ references/ assets/ を認識(その他のディレクトリも可)。
//
// 変換ルール:
// 1. plugins/<name>/skills/<name>/SKILL.md を読む
// 2. frontmatter を name + description のみに整形(Claude 固有の allowed-tools は落とす)
// 3. body はそのまま維持
...
const OUT_ARG = opts.out ?? '.agents/skills';
...
function buildAntigravityFrontmatter(fm) {
  const out = {};
  ...
  // Antigravity 標準は name + description が基本。Claude 固有の allowed-tools は
  // Antigravity が tool ゲートに使わない(実験的フィールド)ため出力しない。
  return out;
}
